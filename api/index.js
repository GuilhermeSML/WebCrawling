...
module.exports = async (req, res) => {
    ...
    const maxCrawlLength = 5; // Reduce the number of crawled pages to avoid timeouts
    let crawledCount = 0;
    const articleData = [];
    ...
    try {
        ...
        while (urlsToVisit.length > 0 && crawledCount < maxCrawlLength) {
            // Limit concurrent requests (e.g., 2 at a time to prevent timeout)
            const currentBatch = urlsToVisit.splice(0, 2);
            const fetchPromises = currentBatch.map(async (url) => {
                ...
                try {
                    const response = await axios.get(url, {
                        timeout: 8000, // Slightly shorter timeout for individual requests to avoid hanging
                        headers: {
            ...
            // Delay between batches to avoid overwhelming external sites
